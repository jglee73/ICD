...
#define CH_TYPE__MASTER				1
#define CH_TYPE__SLAVE				2
...
{
	SCX__ASYNC_TIMER_CTRL x_timer__stable_delay_master;
	SCX__ASYNC_TIMER_CTRL x_timer__check_delay_master;
	SCX__ASYNC_TIMER_CTRL x_timer__change_delay_master;

	SCX__ASYNC_TIMER_CTRL x_timer__stable_delay_slave;
	SCX__ASYNC_TIMER_CTRL x_timer__check_delay_slave;
	SCX__ASYNC_TIMER_CTRL x_timer__change_delay_slave;

	double pre__set_master = -1.0;
	double pre__set_slave  = -1.0;

	CString var_data;

	// ...
	{
		x_timer__stable_delay_master->REGISTER__COUNT_CHANNEL_NAME(sCH__MON_STABLE_TIME_MASTER->Get__CHANNEL_NAME());
		x_timer__check_delay_master->REGISTER__COUNT_CHANNEL_NAME(sCH__MON_FAULT_TIME_MASTER->Get__CHANNEL_NAME());
		x_timer__change_delay_master->REGISTER__COUNT_CHANNEL_NAME(sCH__MON_CHANGE_TIME_MASTER->Get__CHANNEL_NAME());

		x_timer__stable_delay_slave->REGISTER__COUNT_CHANNEL_NAME(sCH__MON_STABLE_TIME_SLAVE->Get__CHANNEL_NAME());
		x_timer__check_delay_slave->REGISTER__COUNT_CHANNEL_NAME(sCH__MON_FAULT_TIME_SLAVE->Get__CHANNEL_NAME());
		x_timer__change_delay_slave->REGISTER__COUNT_CHANNEL_NAME(sCH__MON_CHANGE_TIME_SLAVE->Get__CHANNEL_NAME());
	}

	// ...
	{
		x_timer__stable_delay_master->STOP_ZERO();
		x_timer__check_delay_master->STOP_ZERO();
		x_timer__change_delay_master->STOP_ZERO();

		x_timer__stable_delay_slave->STOP_ZERO();
		x_timer__check_delay_slave->STOP_ZERO();
		x_timer__change_delay_slave->STOP_ZERO();
	}
	...
	while(1)
	{
		...
		for(i=0; i<i_limit; i++)
		{
			if(i == 0)
			{
				ch_type = CH_TYPE__MASTER;

				p_timer__change_delay = x_timer__change_delay_master.Get__PTR();
				p_timer__stable_delay = x_timer__stable_delay_master.Get__PTR();
				p_timer__check_delay  = x_timer__check_delay_master.Get__PTR();

				p_ch__change = sCH__MON_CHANGE_STATE_MASTER.Get__PTR();
				p_ch__stable = sCH__MON_STABLE_STATE_MASTER.Get__PTR();
				p_ch__fault  = sCH__MON_FAULT_STATE_MASTER.Get__PTR();

				cur__ch_set = aCH__PARA_SET_RATIO_MASTER->Get__VALUE();
				p_pre__ch_set = &pre__set_master;
			}
			else if(i == 1)
			{
				ch_type = CH_TYPE__SLAVE;

				p_timer__change_delay = x_timer__change_delay_slave.Get__PTR();
				p_timer__stable_delay = x_timer__stable_delay_slave.Get__PTR();
				p_timer__check_delay  = x_timer__check_delay_slave.Get__PTR();

				p_ch__change = sCH__MON_CHANGE_STATE_SLAVE.Get__PTR();
				p_ch__stable = sCH__MON_STABLE_STATE_SLAVE.Get__PTR();
				p_ch__fault  = sCH__MON_FAULT_STATE_SLAVE.Get__PTR();

				cur__ch_set = aCH__PARA_SET_RATIO_SLAVE->Get__VALUE();
				p_pre__ch_set = &pre__set_slave;
			}
			else
			{
				continue;
		...
		if((sCH__MON_STABLE_STATE_MASTER->Check__DATA(STR__YES) > 0)
		&& (sCH__MON_STABLE_STATE_SLAVE->Check__DATA(STR__YES) > 0))
		{
		...
		if((sCH__MON_FAULT_STATE_MASTER->Check__DATA(STR__YES) > 0)
		|| (sCH__MON_FAULT_STATE_SLAVE->Check__DATA(STR__YES) > 0))
		{
...
::Fnc__STABLE_CHECK(CII_OBJECT__ALARM* p_alarm, const int ch_type, const int alm_report)
{
	...
	if(ch_type == CH_TYPE__MASTER)
	{
		alm_id__warning = ALID__CH1_WARNING;
		alm_id__fault   = ALID__CH1_FAULT;

		cur_set = aCH__PARA_SET_RATIO_MASTER->Get__VALUE();
		cur_get = atof(sCH__MON_GET_RATIO_MASTER->Get__STRING());
	}
	else if(ch_type == CH_TYPE__SLAVE)
	{
		alm_id__warning = ALID__CH2_WARNING;
		alm_id__fault   = ALID__CH2_FAULT;

		cur_set = aCH__PARA_SET_RATIO_SLAVE->Get__VALUE();
		cur_get = atof(sCH__MON_GET_RATIO_SLAVE->Get__STRING());
	}
	else
	{
		return 1;
