	...
	while(1)
	{
		...
		if(sCH__TAS_RESET_REQ->Check__DATA("") < 0)
		{
			...
			for(i=0; i<CFG_ACT__SIZE; i++)
			{
				...
				// ST2 ...
				{
					sCH__TAS_ACTION_TIME_NOW__ST2[i]->Set__DATA("");
					sCH__TAS_ACTION_TIME_MIN__ST2[i]->Set__DATA("");
					sCH__TAS_ACTION_TIME_MAX__ST2[i]->Set__DATA("");
					sCH__TAS_ACTION_TIME_AVG__ST2[i]->Set__DATA("");
					sCH__TAS_ACTION_TIME_AVG_F__ST2[i]->Set__DATA("");
					aCH__TAS_ACTION_TIME_CNT__ST2[i]->Set__VALUE(1);
				}
			}
		}
