...
//-------------------------------------------------------------------------
CObj__IO_SIMPLE::CObj__IO_SIMPLE()
{
...
//-------------------------------------------------------------------------
int CObj__IO_SIMPLE::__DEFINE__CONTROL_MODE(obj,l_mode)
{
	sObject_Name = obj;
...
int CObj__IO_SIMPLE::__DEFINE__VERSION_HISTORY(version)
{
...
int CObj__IO_SIMPLE::__DEFINE__VARIABLE_STD(p_variable)
{
...
int CObj__IO_SIMPLE::__DEFINE__ALARM(p_alarm)
{
	DECLARE__ALARM;
...
int CObj__IO_SIMPLE::__Define__USER_FUNCTION(CII_DEFINE__FUNCTION *p_fnc_ctrl)
{
...
//-------------------------------------------------------------------------
int CObj__IO_SIMPLE::__INITIALIZE__OBJECT(p_variable,p_ext_obj_create)
{
	// ...
	{
		CString file_name;
	...
	// IO : VLV ...
	{
		def_name = "DATA.PUMP_VALVE.SIZE";
		p_ext_obj_create->Get__DEF_CONST_DATA(def_name,def_data);

		iDATA__PUMP_VLV_SIZE = atoi(def_data);
		if(iDATA__PUMP_VLV_SIZE > CFG_SIZE__PUMP_VALVE)			iDATA__PUMP_VLV_SIZE = CFG_SIZE__PUMP_VALVE;

		for(int k=0; k<iDATA__PUMP_VLV_SIZE; k++)
		{
			def_name.Format("CH.PUMP_VALVE.%1d", k + 1);
			p_ext_obj_create->Get__DEF_CONST_DATA(def_name,def_data);
			p_ext_obj_create->Get__CHANNEL_To_OBJ_VAR(def_data, obj_name,var_name);

			LINK__EXT_VAR_DIGITAL_CTRL(diEXT_CH__PUMP_VLV_X[k], obj_name,var_name);
		}
	}
...
//-------------------------------------------------------------------------
int CObj__IO_SIMPLE::__CALL__CONTROL_MODE(mode,p_debug,p_variable,p_alarm)
{
	// ...
	{
		CString log_msg;
...
int CObj__IO_SIMPLE::__CALL__MONITORING(id,p_variable,p_alarm)
{
	switch(id)
	{
