...
int CObj__LBx_CHM_SLOT::__INITIALIZE__OBJECT(p_variable,p_ext_obj_create)
{
	...
	// DB OBJ ...
	{
		...
		str_name = "CFG.SETUP.PRESSURE.CHECK";
		LINK__EXT_VAR_DIGITAL_CTRL(dEXT_CH__CFG_SETUP_PRESSURE_CHECK, def_data,str_name);
	...
	for(i=0; i<iLBx_SLOT_SIZE; i++)
	{
		...
		// SV ...
		{
			...
			// DO : UP & DOWN
			{
				def_name.Format("CH__IO_DO_SV_TM_UP.%1d", id);
				p_ext_obj_create->Get__DEF_CONST_DATA(def_name, def_data);

				def_check = x_utility.Check__Link(def_data);
				bActive_DO__LLx_SV_UP_X[i] = def_check;

				if(def_check)
				{
					p_ext_obj_create->Get__CHANNEL_To_OBJ_VAR(def_data, obj_name,var_name);
					LINK__EXT_VAR_DIGITAL_CTRL(doEXT_CH__LLx_SV_UP_X[i], obj_name,var_name);
				}

				//
				def_name.Format("CH__IO_DO_SV_TM_DOWN.%1d", id);
				p_ext_obj_create->Get__DEF_CONST_DATA(def_name, def_data);

				def_check = x_utility.Check__Link(def_data);
				bActive_DO__LLx_SV_DOWN_X[i] = def_check;

				if(def_check)
				{
					p_ext_obj_create->Get__CHANNEL_To_OBJ_VAR(def_data, obj_name,var_name);
					LINK__EXT_VAR_DIGITAL_CTRL(doEXT_CH__LLx_SV_DOWN_X[i], obj_name,var_name);
				}
			}
			// DI : UP & DOWN
			{
				def_name.Format("CH__IO_DI_SV_TM_UP.%1d", id);
				p_ext_obj_create->Get__DEF_CONST_DATA(def_name, def_data);

				def_check = x_utility.Check__Link(def_data);
				bActive_DI__LLx_SV_UP_X[i] = def_check;

				if(def_check)
				{
					p_ext_obj_create->Get__CHANNEL_To_OBJ_VAR(def_data, obj_name,var_name);
					LINK__EXT_VAR_DIGITAL_CTRL(diEXT_CH__LLx_SV_UP_X[i], obj_name,var_name);
				}

				//
				def_name.Format("CH__IO_DI_SV_TM_DOWN.%1d", id);
				p_ext_obj_create->Get__DEF_CONST_DATA(def_name, def_data);

				def_check = x_utility.Check__Link(def_data);
				bActive_DI__LLx_SV_DOWN_X[i] = def_check;

				if(def_check)
				{
					p_ext_obj_create->Get__CHANNEL_To_OBJ_VAR(def_data, obj_name,var_name);
					LINK__EXT_VAR_DIGITAL_CTRL(diEXT_CH__LLx_SV_DOWN_X[i], obj_name,var_name);
				}
			}
		}
	}
	...
	// GAUGE IO OBJECT ...
	{
		...
		// ATM_SNS.IO
		{
			...
			p_ext_obj_create->Get__CHANNEL_To_OBJ_VAR(def_data, obj_name,var_name);
			LINK__EXT_VAR_DIGITAL_CTRL(diEXT_CH__LLx_ATM_SNS, obj_name,var_name);
		}

		// VAC_SNS.VIRTUAL
		{
			def_name = "DATA.VAC_SNS.VIRTUAL_TYPE";
			p_ext_obj_create->Get__DEF_CONST_DATA(def_name,def_data);

			if(def_data.CompareNoCase("YES") == 0)		bActive__VAC_SNS_Virtual_Type = true;
			else										bActive__VAC_SNS_Virtual_Type = false;
		}
		// VAC_SNS.IO
		{
			...
			p_ext_obj_create->Get__CHANNEL_To_OBJ_VAR(def_data, obj_name,var_name);
			LINK__EXT_VAR_DIGITAL_CTRL(diEXT_CH__LLx_VAC_SNS, obj_name,var_name);
		}
	}
