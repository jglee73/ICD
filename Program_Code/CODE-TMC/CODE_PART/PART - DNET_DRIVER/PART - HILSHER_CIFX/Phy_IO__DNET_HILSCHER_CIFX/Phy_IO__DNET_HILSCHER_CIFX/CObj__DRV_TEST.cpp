...
//--------------------------------------------------------------------------------
int CObj__DRV_TEST::__DEFINE__CONTROL_MODE(obj, l_mode)
{
	sObject_Name = obj;

	// ...
	{
		ADD__CTRL_VAR(sMODE__DRIVER_OPEN,   "DRIVER.OPEN");
		ADD__CTRL_VAR(sMODE__ENUM_BOARD,    "ENUM.BOARD");
		ADD__CTRL_VAR(sMODE__SYS_DEVICE,    "SYS.DEVICE");
		ADD__CTRL_VAR(sMODE__CHANNEL_INFO,  "CHANNEL.INFO");
		ADD__CTRL_VAR(sMODE__DEVICE_RESET,  "DEVICE.RESET");
		ADD__CTRL_VAR(sMODE__DRIVER_CLOSE,  "DRIVER.CLOSE");

		//
		ADD__CTRL_VAR(sMODE__EXPLICIT_MESSAGE, "EXPLICIT.MESSAGE");
...
int CObj__DRV_TEST::__DEFINE__VERSION_HISTORY(version)
{
	version = "JG.Lee \n";
...
int CObj__DRV_TEST::__DEFINE__VARIABLE_STD(p_variable)
{
	...
	// MON  ...
	{
		...
		STD__ADD_STRING(str_name);
		LINK__VAR_STRING_CTRL(dCH__MON_COMM_STATE, str_name);

		str_name = "MON.ACTIVE.DNET_START";
		STD__ADD_DIGITAL_WITH_X_OPTION(str_name, "OFF ON", "");
		LINK__VAR_DIGITAL_CTRL(dCH__MON_ACTIVE_DNET_START, str_name);
	}
	...
	// PARA ...
	{
		...
		str_list  = STR__PRODUCT_NAME;
		str_list += " ";
		str_list += STR__IN_BYTE;
		str_list += " ";
		str_list += STR__OUT_BYTE;

		str_name = "PARA.SLAVE.REQ.INFO";
		STD__ADD_DIGITAL_WITH_X_OPTION(str_name, str_list, "");
		LINK__VAR_DIGITAL_CTRL(dCH__PARA_SLAVE_REQ_INFO, str_name);

		str_name = "PARA.SLAVE.REQ.MAC_ID";
		STD__ADD_ANALOG_WITH_X_OPTION(str_name, "id", 0, 1, 63, "");
		LINK__VAR_ANALOG_CTRL(aCH__PARA_SLAVE_REQ_MAC_ID, str_name);

		//
		str_name = "PARA.SLAVE.RSP.STATE";
		STD__ADD_DIGITAL_WITH_X_OPTION(str_name, "UNKNOWN OK ERROR", "");
		LINK__VAR_DIGITAL_CTRL(dCH__PARA_SLAVE_RSP_STATE, str_name);

		str_name = "PARA.SLAVE.RSP.DATA.HEXA";
		STD__ADD_STRING_WITH_X_OPTION(str_name, "");
		LINK__VAR_STRING_CTRL(sCH__PARA_SLAVE_RSP_DATA_HEXA, str_name);

		str_name = "PARA.SLAVE.RSP.DATA.1";
		STD__ADD_STRING_WITH_X_OPTION(str_name, "");
		LINK__VAR_STRING_CTRL(sCH__PARA_SLAVE_RSP_DATA_1, str_name);

		str_name = "PARA.SLAVE.RSP.DATA.2";
		STD__ADD_STRING_WITH_X_OPTION(str_name, "");
		LINK__VAR_STRING_CTRL(sCH__PARA_SLAVE_RSP_DATA_2, str_name);
	}

	// Test.Para ...
	{
		str_name = "TEST.PARA.BOARD_NAME";
		STD__ADD_STRING_WITH_X_OPTION(str_name, "");
		LINK__VAR_STRING_CTRL(sCH__TEST_PARA_BOARD_NAME, str_name);

		str_name = "TEST.PARA.CHANNEL_ID";
		STD__ADD_ANALOG_WITH_X_OPTION(str_name, "ID", 0, 0, 4, "");
		LINK__VAR_ANALOG_CTRL(aCH__TEST_PARA_CHANNEL_ID, str_name);

		//
		str_name = "TEST.PARA.MAC_ID";
		STD__ADD_ANALOG_WITH_X_OPTION(str_name, "ID", 0, 1, 64, "");
		LINK__VAR_ANALOG_CTRL(aCH__TEST_PARA_MAC_ID, str_name);

		str_name = "TEST.PARA.CLASS_ID";
		STD__ADD_ANALOG_WITH_X_OPTION(str_name, "ID", 0, 1, 100, "");
		LINK__VAR_ANALOG_CTRL(aCH__TEST_PARA_CLASS_ID, str_name);

		str_name = "TEST.PARA.INSTANCE_ID";
		STD__ADD_ANALOG_WITH_X_OPTION(str_name, "ID", 0, 1, 10, "");
		LINK__VAR_ANALOG_CTRL(aCH__TEST_PARA_INSTANCE_ID, str_name);

		str_name = "TEST.PARA.ATTRIBUTE_ID";
		STD__ADD_ANALOG_WITH_X_OPTION(str_name, "ID", 0, 1, 255, "");
		LINK__VAR_ANALOG_CTRL(aCH__TEST_PARA_ATTRIBUTE_ID, str_name);
	}

	// Config ...
	{
		str_name = "CFG.DRV_INT.ERROR.CHECK";
		STD__ADD_DIGITAL_WITH_X_OPTION(str_name, "YES  NO", "");
		LINK__VAR_DIGITAL_CTRL(dCH__CFG_DRV_INT_ERROR_CHECK, str_name);
	...
	// ...
	{
		// DNET.INFO ...
		{
			...
			//
			str_name = "DNET.INFO.MASTER_CHANNEL.ID";
			STD__ADD_STRING(str_name);
			LINK__VAR_STRING_CTRL(sCH__DNET_INFO__MASTER_CHANNEL_ID, str_name);
...
int CObj__DRV_TEST::__DEFINE__ALARM(p_alarm)
{
	DECLARE__ALARM;
	...
	// ...
	{
		alm_id = ALID__DNET_NODE_IN_BYTE_UPDATE_ERROR;

		alm_title  = obj_title;
		alm_title += "DeviceNet In-Byte Update Error !";
	...
	// ...
	{
		alm_id = ALID__DNET_NODE_OUT_BYTE_UPDATE_ERROR;

		alm_title  = obj_title;
		alm_title += "DeviceNet Out-Byte Update Error !";
...
int CObj__DRV_TEST::__DEFINE__VARIABLE_IO(p_io_variable)
{
...
//--------------------------------------------------------------------------------
int CObj__DRV_TEST::__INITIALIZE__OBJECT(p_variable,p_ext_obj_create)
{
	CString def_name;
	CString def_data;
	...
	// ...
	{
		...
		mDNet_Mng.Link__LOG_CTRL(xI__APP_LOG_CTRL.Get__SCI_PTR());
	...
	// ...
	{
		bActive__DNET_START = false;
		bActive__POLL_LOOP  = false;
		bActive__EXT_MSG    = false;

		dCH__MON_ACTIVE_DNET_START->Set__DATA(STR__OFF);
	}
	return 1;
}

int CObj__DRV_TEST::__INITIALIZE__IO(p_io_para)
{
	// ...
	{
	...
	// ...
	{
		...
		//
		para_name = "CHANNEL.ID";
		p_io_para->Get__PARAMETER_DATA(para_name, para_data);
		int ch_ud = atoi(para_data);
		mDNet_Mng.Set__CHANNEL_ID(ch_ud);
	...
	if(iActive__SIM_MODE > 0)			r_ret = 1;
	else								r_ret = _Init__DNET_MASTER_BY_USER_CFG();

	if(r_ret > 0)
	{
		printf("%s : Initial Completed ... \n", sObject_Name);

		uDNet_OutputSize = mCtrl__DNet_Node.Get__TOTAL_OUT_BYTE();
		uDNet_InputSize  = mCtrl__DNet_Node.Get__TOTAL_IN_BYTE();
	}
	else
	{
	...
	// ...
	{
		...
		ch_data.Format("%1d", uDNet_OutputSize);
		sCH__DNET_CFG__TOTAL_OUT_BYTE_MAX->Set__DATA(ch_data);

		ch_data.Format("%1d", uDNet_InputSize);
		...
		// ...
		{
			...
			else							ch_data = mDNet_Mng.Get__CHANNEL_FIRMWARE();
		...
		ch_data = mDNet_Mng.Get__CHANNEL_VERSION();
		sCH__DNET_INFO__MASTER_BOARD_DRIVER_VERSION->Set__DATA(ch_data);

		//
		iDNet_ChannelNumber = mDNet_Mng.Get__CHANNEL_ID();
		ch_data.Format("%1d", iDNet_ChannelNumber);
		sCH__DNET_INFO__MASTER_CHANNEL_ID->Set__DATA(ch_data);
	}
...
::__CALL__CONTROL_MODE(mode, p_debug, p_variable, p_alarm)
{
	...
	// ...
	{
			 IF__CTRL_MODE(sMODE__DRIVER_OPEN)					flag = Call__DRIVER_OPEN(p_variable, p_alarm);
		ELSE_IF__CTRL_MODE(sMODE__ENUM_BOARD)					flag = Call__ENUM_BOARD(p_variable, p_alarm);
		ELSE_IF__CTRL_MODE(sMODE__SYS_DEVICE)					flag = Call__SYS_DEVICE(p_variable, p_alarm);
		ELSE_IF__CTRL_MODE(sMODE__CHANNEL_INFO)					flag = Call__CHANNEL_INFO(p_variable, p_alarm);
		ELSE_IF__CTRL_MODE(sMODE__DEVICE_RESET)					flag = Call__DEVICE_RESET(p_variable, p_alarm);
		ELSE_IF__CTRL_MODE(sMODE__DRIVER_CLOSE)					flag = Call__DRIVER_CLOSE(p_variable, p_alarm);

		ELSE_IF__CTRL_MODE(sMODE__EXPLICIT_MESSAGE)				flag = Call__EXPLICIT_MESSAGE(p_variable, p_alarm);
...
int CObj__DRV_TEST::__CALL__MONITORING(id, p_variable, p_alarm)
{
