				 ...
				 double& read_data)
{
	...
	*/
	...
	// ...
	{
		...
		CString data_type1 = io_info.sCOMMAND1;
		CString data_type3 = io_info.sCOMMAND3;

		if((data_type1.CompareNoCase(_DATA_TYPE__GAUGE) == 0)
		|| (data_type3.CompareNoCase(_DATA_TYPE__GAUGE) == 0))
		{
			ai_type   = _AI_TYPE__GAUGE;
			byte_size = 2;
		}
		else if((data_type1.CompareNoCase(_DATA_TYPE__FFFF) == 0)
			 || (data_type3.CompareNoCase(_DATA_TYPE__FFFF) == 0))
		{
			ai_type   = _AI_TYPE__FFFF;
			byte_size = 2;
		}
		else if((data_type1.CompareNoCase(_DATA_TYPE__1F40) == 0)
			 || (data_type3.CompareNoCase(_DATA_TYPE__1F40) == 0))
		{
			ai_type   = _AI_TYPE__1F40;
			byte_size = 2;
		}		
		else if((data_type1.CompareNoCase(_DATA_TYPE__FLOAT) == 0)
			 || (data_type3.CompareNoCase(_DATA_TYPE__FLOAT) == 0))
		{
	...
	*/
	...
	// ...
	{
		...
		if(mCtrl__DNet_Node.Get__IN_BYTE__IO_BYTE(md_id,ch_id,byte_offset,byte_size, uch_data,100) > 0)
		{
			...
			if(data_type.CompareNoCase(_DATA_TYPE__ASCII) == 0)
			{
				read_data.Format("%s", uch_data);
			}
			else if(data_type.CompareNoCase(_DATA_TYPE__FLOAT) == 0)
			{
...
::__Write__ANALOG(const CString& var_name, const CDS_IO__CHANNEL_INFO& io_info, const double set_data)
{
	...
	// ...
	{
		...
		if((data_type1.CompareNoCase(_DATA_TYPE__FFFF) == 0)
		|| (data_type3.CompareNoCase(_DATA_TYPE__FFFF) == 0))
		{
			ao_type   = _AO_TYPE__FFFF;
			byte_size = 2;
		}

		if(ao_type == _AO_TYPE__FFFF)
		{
			UNION_2_BYTE__UINT m_data;

			int set_hexa = (int) (set_data * 10.0);
			if(set_hexa < 0)
			{
				set_hexa = -set_hexa;
				set_hexa = 0x0FFFF - set_hexa;
			}
